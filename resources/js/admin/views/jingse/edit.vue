<script>
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          title: [
            {required: true, message: '请输入景点标题', trigger: 'change'}
          ],
          describe: [
    ...
    async beforeCreate () {
      let {data} = await this.$http.get(this.$api.jingse.show(this.$route.params.id))
      this.form = data
      this.inited  = true
    },
    methods: {
      store() {
        this.$refs['form'].validate((valid) => {
          ...
          this.loading = true
          this.$http.put(this.$api.jingse.update(this.$route.params.id), this.form)
          .then(r => {
            this.loading = false
